...
void main() //void : return값 없는 자료형 함수 main() : C언어 시작시 가장 먼저 호출되는 함수
{
	...
	int		num;
	int		bae;
	int		count = 0; // = :  단순대입 연산자이며 = 왼쪽의 변수에 오른쪽의 값을 대입할 때 사용 Ex) a = 10 -> a는 10이다
	...
	{ 
		if (n % bae == 0) //if문 : if문 ()내 주어진 조건식을 판단하여 참일 경우에만 실행문을 실행 if (조건식) {실행문;}으로 사용, == :  관계연산자이며 a == b의 뜻은 a와 b가 같으면 참이라는 의미이다.
			printf("%d\t", n); //\t : Tab 키와 동일한 기능을 하는 제어문자
			count++; // ++: 증감연산자이며 a++의 뜻은 실행문을 실행한 후에 a의 값을 1 증가하라는 의미이다.
	} printf("이며 총 %d 개 입니다\n", count); //printf 함수 내 " " 안의 출력할 문자열에 형식지정문자 (Ex:%d)를 삽입하고 변수명은 " " 뒤에 ,로 구분하여 지정한다 변수와 형식지정문자는 1대 1로 매핑된다
