...
void main() //void : return값 없는 자료형 함수 main() : C언어 시작시 가장 먼저 호출되는 함수
{
	...
	printf("첫번째 수를 입력하세요: ");
	scanf("%d", &nu1); // scanf: 표준입력함수이며 키보드를 통해서 데이터를 입력하는 기능을 제공한다 & 변수는 메모리 공간에서의 변수에 대한 주소를 구하기 위해 변수명 앞에 반드시 선언해야 한다
	...
	for (n = nu1; n <= nu2; n++)// for문 : 반복 제어문이며 for(초기화식; 조건식; 증감식;) 으로 구성 
	...
	{ // ++: 증감연산자이며 a++의 뜻은 실행문을 실행한 후에 a의 값을 1 증가하라는 의미이다.
		sum += n; // += : 복합대입연산자이며 a += b의 뜻은 a에 a와 b를 더한 결과를 a에 저장하라는 의미이다..
	} // <= :  관계연산자이며 a <= b의 뜻은 b가 a보다 크거나 같으면 참이라는 의미이다.

	printf("%d부터 %d까지의 합은 %d입니다\n", nu1, nu2, sum); //printf() : 프로그램에서 모니터를 통하여 글자를 출력하는 기능을 제공함, \n : 줄을 바꾸는 제어문자
