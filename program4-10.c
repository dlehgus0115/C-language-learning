...
void main() //void : return값 없는 자료형 함수 main() : C언어 시작시 가장 먼저 호출되는 함수
{
	...
	if (num == 0) //다중 if-else문 : if문 ()내 주어진 여러 개의 조건식을 판단하여 참일 경우에 실행할 실행문과 거짓일 경우 실행할 여러 개의 실행문을 설정가능 
		printf("입력하신 숫자는 0 입니다\n"); //if (조건식)  실행문;  else if (조건식) 실행문 ···else (조건식) 실행문 으로 사용
	else if (num < 0)
		printf("숫자 %d는 음수 입니다\n", num); //printf() : 프로그램에서 모니터를 통하여 글자를 출력하는 기능을 제공함, \n : 줄을 바꾸는 제어문자
	else if (num % 2 == 0)
		printf("숫자 %d는 짝수 입니다\n", num); //printf 함수 내 " " 안의 출력할 문자열에 형식지정문자 (Ex:%d)를 삽입하고 변수명은 " " 뒤에 ,로 구분하여 지정한다 변수와 형식지정문자는 1대 1로 매핑된다
	else if (num % 2 == 1)
		printf("숫자 %d는 홀수 입니다\n", num); //%d : 정수형 형식지정문자이며 부호 있는 10진수 형태의 숫자를 출력할 때 사용한다
}