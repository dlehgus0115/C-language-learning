...
{
	int		num, n;//int : 4바이트 정수형을 처리하기 위한 자료형이며 -214783648부터 2147483647까지 정수를 저장가능 변수 선언시 int 변수명 과 같이 선언
	int		i, j;
	printf("수를 입력하세요: ");
	...
	for (i = 2; i <= num; i++) { // for문 : 반복 제어문이며 for(초기화식; 조건식; 증감식;) 으로 구성 
		// 초기화식 : for 문 실행될 때 처음에 한번만 실행, 조건식 : 거짓일 시 실행문 블록 미실행 후 다음 코드 실행 참일 시 실행문 블록 실행 및 증감식 실행
		for (j = 2; j <= i; j++) {
			if (i % j == 0) //if문 : if문 ()내 주어진 조건식을 판단하여 참일 경우에만 실행문을 실행 if (조건식) {실행문;}으로 사용
				break;  // break : switch, while, for문에서 빠져나오는 기능을 수행 혹은 switch 문의 괄호에서 빠져나오는 기능을 수행
		}
		if (i == j) //== :  관계연산자이며 a == b의 뜻은 a와 b가 같으면 참이라는 의미이다.
			printf("%d\t, i"); //printf() : 프로그램에서 모니터를 통하여 글자를 출력하는 기능을 제공함, \t : Tab 키와 동일한 기능을 하는 제어문자
	}
	printf("입니다\n"); //\n : 줄을 바꾸는 제어문자
}
