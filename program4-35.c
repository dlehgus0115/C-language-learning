...
void main() //void : return값 없는 자료형 함수 main() : C언어 시작시 가장 먼저 호출되는 함수
{
	...
	for (n = 2; n <= num; n++)// for문 : 반복 제어문이며 for(초기화식; 조건식; 증감식;) 으로 구성 
	...
	if (n == num)  //if-else문 : if문 ()내 주어진 조건식을 판단하여 참일 경우에 실행할 실행문과 거짓일 경우  실행할 실행문을 설정가능 if (조건식) {실행문;} else {실행문}으로 사용
	{ // == :  관계연산자이며 a == b의 뜻은 a와 b가 같으면 참이라는 의미이다.
		printf("%d 는 소수입니다.\n", num); //if() 조건식이 [참] 일 떄만 실행
