...
void main() //void : return값 없는 자료형 함수 main() : C언어 시작시 가장 먼저 호출되는 함수
{
	...
	printf("점수를입력하세요 :");
	scanf("%d", &su); // scanf: 표준입력함수이며 키보드를 통해서 데이터를 입력하는 기능을 제공한다 & 변수는 메모리 공간에서의 변수에 대한 주소를 구하기 위해 변수명 앞에 반드시 선언해야 한다
	//사용시 scanf 함수 내 " " 안의 출력할 문자열에 형식지정문자 (Ex:%d)를 삽입하고 변수명은 " "안의 형식지정문자 뒤에 ,&변수명으로 지정한다 변수와 형식지정문자는 1대 1로 매핑된다
	if (su > 95)  //if-else문 : if문 ()내 주어진 조건식을 판단하여 참일 경우에 실행할 실행문과 거짓일 경우  실행할 실행문을 설정가능 if (조건식) {실행문;} else {실행문}으로 사용
	{
		printf("우수한 성적입니다.\n"); //if() 조건식이 [참] 일 떄만 실행
	}
	else
